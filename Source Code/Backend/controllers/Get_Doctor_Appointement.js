...
exports.Get_Doctor_Appointement = async (req, res) => {
  try {
    ...
    const title = `Request For Appointment Mail`;
    const body = `${patient.name} , Your request for the appointment with Doctor ${doctor.name} on ${date} has been sent successfully.<br> You will get confirmation mail and location once doctor approved it  `
    //email,title,body

    await mailSender(patient.email,title,body);
